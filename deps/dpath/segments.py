...
def set(obj, segments, value, creator=__default_creator__, hints=()):
    ...
    # print(f"Old Current: {current}, Old Segments {type(segments[-1])}, Old Value: {value}.")
    ...
    # Custom list handler
        ...
        if isinstance(current, list):
            if None not in current:
                current.append(None)
            segments[-1] = current.index(segments[-1] != 'None' and segments[-1] or None)


    # print(f"Current: {current}, Segments {segments[-1]}, Value: {value}.")
